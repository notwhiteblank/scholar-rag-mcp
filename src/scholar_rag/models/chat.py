...
class ChatClient(OpenAICompatClient):
    def __init__(
        self,
        *,
        backend: str = "api",
        ...
        if backend == "local":
            raise NotImplementedError("local chat backend is not implemented")
        super().__init__(
            base_url=base_url,
            api_key=api_key,
            model=model,
    ...
    def _complete_plain(self, prompt: str) -> str:
        ...
        if response.status_code >= 400:
            raise ServiceUnavailableError(
                _status_error_message("chat completion", response.status_code, self.max_retries)
            )
        content = _extract_content(response)
        if content is None:
